...
def apply_rope_torch_reference(
    ...
    """Unoptimized reference path for honest benchmark comparisons."""
    import torch
    ...
    q_out = q * torch.cos(phase) + rotate_half_t(q) * torch.sin(phase)
    k_out = k * torch.cos(phase) + rotate_half_t(k) * torch.sin(phase)
    return q_out, k_out
